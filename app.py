...
class Plant_Eval:
    ...
    def plant_Eval_Analytics(self):
        ...
        plt.plot(df["x_points"], df["I2"], label="Line Current A")
        ...
        plt.grid()
        plt.plot(df['x_points'], df['Psum_kW'], label='Power consumption profile')
        plt.legend()
...
# =============================================================================
    ...
    def Quote_printOut(self):
        ...
        # Save the workbook in a specific folder
        folder_path = os.path.join('quotes', self.plant_eval_name)
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        excel_file_path = os.path.join(folder_path, self.plant_eval_name + '.xlsx')
...
def analyze():
    if request.method == 'POST':
        ...
        plant = System_Builder(load_data_df, plant_name, battery_hours, distance)
        ...
        key_metrics = {
            ...
            'max_power':Plant_Eval().get_max_power(),
            # ... other key metrics
        }
...
def upload_file():
    ...
    if file:
        ...
        battery_hours = int(request.form['battery_hours'])
        distance = int(request.form['distance'])
        plant = System_Builder(df, plant_name, battery_hours, distance)
        #clean_data = plant.plant_eval_data_cleanup()
        plant.plant_ETB_data()
        analytics_image = plant.plant_Eval_Analytics()
        
        file_path = plant.Quote_printOut()

        #return render_template('result.html', image=system_builder.get_key_metrics(), pdf_path=pdf_file_path)
        
        return render_template('result.html', pdf_path=file_path, image=analytics_image)
